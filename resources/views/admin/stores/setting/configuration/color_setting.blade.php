  ...
  <div class="row">
    <div class="col-12">
      <ul class="nav nav-tabs customtab2" role="tablist">
        ...
        <li class="nav-item"> <a class="nav-link active" href="{{route('admin.colorSettingIndexConfiguration')}}" role="tab"><span class="hidden-sm-up"><i class="ion-contrast"></i></span> <span class="hidden-xs-down">{{__('main.color_setting')}}</span> </a> </li>
        <li class="nav-item">
      ...
      <div class="tab-content">
        <div class="tab-pane active" id="color-setting" role="tabpanel">
          <div class="row">
            <div class="col-xl-12">
              <div class="box box-bordered border-primary m-10">
                <div class="box-header with-border">
                  <h4 class="box-title text-primary"><strong>{{ __('main.color_setting') }}</strong></h4>
                </div>
                <div class="box-body">
                  <form class="form" method="POST" action="{{ route('admin.colorSettingConfiguration') }}"
                    enctype="multipart/form-data">
                    @csrf
                      <div class="form-group row">
                        <div class="col-6">
                          <label for="example-color-input" class="form-label">{{__('main.button_color_setting')}}</label>
                          <div class="col-sm-10">
                          <input class="form-control" name="data[button_color]" type="color" @if (isset($colorSetting->data->button_color)) value="{{$colorSetting->data->button_color}}" @else value="#563d7c" @endif id="example-color-input">
                          </div>
                        </div>
                        <div class="col-6">
                          <label for="example-color-input" class="form-label">{{__('main.toast_success_color')}}</label>
                          <div class="col-sm-10">
                          <input class="form-control" name="data[toast_success_color]" type="color" @if (isset($colorSetting->data->toast_success_color)) value="{{$colorSetting->data->toast_success_color}}" @else value="#563d7c" @endif id="example-color-input">
                          </div>
                        </div>
                      </div>
                      <div class="form-group row">
                        <div class="col-6">
                          <label for="example-color-input" class="form-label">{{__('main.toast_error_color')}}</label>
                          <div class="col-sm-10">
                          <input class="form-control" name="data[toast_error_color]" type="color" @if (isset($colorSetting->data->toast_error_color)) value="{{$colorSetting->data->toast_error_color}}" @else value="#563d7c" @endif id="example-color-input">
                          </div>
                        </div>
                        <div class="col-6">
                          <label for="example-color-input" class="form-label">{{__('main.toast_warning_color')}}</label>
                          <div class="col-sm-10">
                          <input class="form-control" name="data[toast_warning_color]" type="color" @if (isset($colorSetting->data->toast_warning_color)) value="{{$colorSetting->data->toast_warning_color}}" @else value="#563d7c" @endif id="example-color-input">
                          </div>
                        </div>
                      </div>
